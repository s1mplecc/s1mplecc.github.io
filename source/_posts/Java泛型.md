...
> 我的上一篇博客 [Java泛型与擦除](https://s1mple.xyz/java-generic-type-erasure/)只是对于 Java 泛型和类型擦除做了一个简要的介绍，本文主要是针对 Java 泛型做一些更深入的补充说明。
...
> 在 Java SE 7之后，允许我们使用如下更简洁的语法`Box<Integer> integerBox = new Box<>();`，因为编译器能够推断出上下文的类型参数，所以没必要做冗余的声明
...
- 一个典型案例就是 Map，它存储一对键值对，我截取了一段 Map.Entry 的源码
