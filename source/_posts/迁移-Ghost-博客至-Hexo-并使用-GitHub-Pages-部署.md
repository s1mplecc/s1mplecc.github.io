...
```sh
➜  github.io git:(hexo) ✗ git branch -v    
...
现在你可以使用 `hexo` 命令了，在生成 Hexo 项目前需要确保一件事：你已将 Git 仓库 clone 至本地，并且处于 hexo 分支上。由于只能在**空文件夹**下生成 Hexo 项目，所以我们先将 `.git` 文件夹整个移出去，然后执行 `hexo init` 命令。最后别忘了将 `.git` 文件夹移回来。
