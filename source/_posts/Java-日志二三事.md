...
![0a9ca27a6fcb038fca91153b1147e5cc.jpeg](0.jpg)
...
下图是 [SLF4J官网](https://www.slf4j.org/manual.html) 介绍可以绑定的日志实现框架。其中 slf4j-simple 是为小项目提供的简单实现；logback-classic 是官方的原生实现，不需要额外的适配器。而 `slf4j-log4j12` 和 `slf4j-jdk14` 分别是适配到 Log4j 和 JUL 的依赖，JUL 由于是 JDK 自带所以不需要额外依赖，而 Log4j 则还需要添加自己的底层实现依赖。

![b7d8f119aca4c91e4d72283f7198148f.png](1.png)

下面这张图展示了 SLF4J 绑定不同日志实现框架需要的依赖：

![d8261ab119309917e25180ef0ae42abe.png](2.png)
...
而 Log4j2 的优点则在于性能，在 [Is it worth to use slf4j with log4j2](https://stackoverflow.com/questions/41498021/is-it-worth-to-use-slf4j-with-log4j2) 这个问题中推荐直接面向 Log4j2 API 编程，理由如下：
...
![900a0952b7540882925dd50699dc3e29.png](3.png)

![c5b42a899b63e265a3e2f4300d443ecb.png](4.png)
...
具体来说，现在**推荐使用 Log4j2 API 或者 SLF4j**，不推荐继续使用 JCL。
...
设为 optional，**依赖不会传递**，这样如果你的项目被别的项目依赖，它就不会引入不想要的 Log Implementation 依赖，即**使用你提供的库的用户可以自定义 Log Implementation**；

Scope 设置为 runtime，是为了防止开发人员在项目中直接使用 Log Implementation 中的类，而不适用 Log Facade 中的类，即**编码时程序员只可见 Log Facade 层面而不必关注实现层面**。
...
这种情况下，推荐的处理方法，是**使用 exclude 来排除所有的这些 Log 实现和桥接器的依赖，只保留第三方库里面对 Log Facade 的依赖**。
...
- [面向log4j2 API编程而不是slf4j](http://codepub.cn/2017/03/08/Programming-to-the-log4j2-API-instead-of-slf4j/)
- [SLF4J 官方文档](https://www.slf4j.org/manual.html)
- [Apache Log4j2 官方文档](https://logging.apache.org/log4j/2.x/performance.html)