...
更确切的说，Optional 是一个 **Monad** 容器。那什么是 Monad 呢？可以参考下面这段话：` Think of monads as an object that wraps a value and allows us to apply a set of transformations on that value and get it back out with all the transformations applied.` 简单的说，Monad 是一个包裹了一个值的容器（值可以是单个对象也可以是集合），允许我们对该值进行一系列的转换（函数操作）后返回给我们期望的值。也就是说，Monad 封装了接收函数作为入参的一些方法（filter、map、flatMap 等）。

那么现在，让我们从源码开始一步一步揭开它的神秘面纱。

## 工厂方法

Optional 使用私有化的构造函数和单例模式提供了**值为空的单例**，暴露的静态工厂方法为 `Optional.empty()`。该模式有点类似 Java 设计模式中的**空对象模式**，也是避免空指针的核心所在，后面我们将会提到。
